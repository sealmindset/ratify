...
def _is_trusted_url(url: str) -> bool:
    """Validate that a URL is from a trusted OIDC source.

    In local dev, mock-oidc uses an internal hostname for token/userinfo endpoints
    but returns localhost for the browser-facing authorization endpoint.
    Both are trusted.
    """
    ...
    if parsed.scheme not in ("http", "https"):
        return False
    trusted_hosts = {issuer_parsed.hostname, "localhost", "127.0.0.1"}
